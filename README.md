...
### If the `plab` command is not found
...
the launcher should be available as:

```bash
plab
```

If your shell does not find it, run PulsarLab directly with:

```bash
python -m pulsarlab
```

This uses the same application entry point and is useful on systems where the Python scripts directory is not in `PATH`.
